...
use m68000::instruction::{
    displacement, immediate, rotation_direction_size_mode_register,
    size_effective_address_immediate, vector,
};
...
pub fn criterion_benchmark(c: &mut Criterion) {
    {
        ...
        c.bench_function("displacement byte", |b| {
            b.iter(|| {
...

    {
        ...
        c.bench_function("displacement word", |b| {
            b.iter(|| {
                let mut memory_iter = code_bra.iter_u16(2).unwrap();
                black_box(displacement(black_box(bra), &mut memory_iter));
            })
...

    {
        ...
        c.bench_function("size_effective_address_immediate", |b| {
            b.iter(|| {
                let mut memory_iter = code_andi.iter_u16(2).unwrap();
                black_box(size_effective_address_immediate(
                    black_box(andi),
                    &mut memory_iter,
                ));
            })
    ...
    {
        let lsr = 0xE088_u16;
        c.bench_function("rotation_direction_size_mode_register", |b| {
            b.iter(|| {
