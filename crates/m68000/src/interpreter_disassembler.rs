...
impl<CPU: CpuDetails> M68000<CPU> {
    ...
    fn get_next_instruction<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
    ) -> Result<Instruction, u8> {
    ...
    pub fn disassembler_interpreter<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
    ) -> (u32, String, usize) {
    ...
    pub fn disassembler_interpreter_exception<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
    ) -> (u32, String, usize, Option<u8>) {
        if self.stop {
    ...
    fn instruction_unknown_instruction<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        _: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_abcd<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_add<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_adda<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_addi<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_addq<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_addx<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_and<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_andi<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_andiccr<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_andisr<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_asm<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_asr<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_bcc<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_bchg<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_bclr<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_bra<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_bset<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_bsr<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_btst<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_chk<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_clr<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_cmp<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_cmpa<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_cmpi<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_cmpm<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_dbcc<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    /// process_exception method returns the exception processing time.
    fn instruction_divs<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    /// If a zero divide exception occurs, this method returns the effective address calculation time, and the
    /// process_exception method returns the exception processing time.
    fn instruction_divu<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_eor<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_eori<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_eoriccr<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_eorisr<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_exg<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_ext<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_illegal<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        _: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_jmp<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_jsr<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_lea<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_link<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_lsm<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_lsr<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_move<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_movea<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_moveccr<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_movefsr<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_movesr<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_moveusp<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_movem<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_movep<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
        let (data, dir, size, addr, disp) = inst
            .operands
            .register_direction_size_register_displacement();
        self.execute_movep(memory, data, dir, size, addr, disp)
    }

    fn instruction_moveq<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_muls<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_mulu<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_nbcd<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_neg<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_negx<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_nop<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        _: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_not<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_or<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_ori<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_oriccr<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_orisr<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_pea<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_reset<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        _: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_rom<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_ror<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_roxm<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_roxr<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_rte<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        _: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_rtr<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        _: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_rts<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        _: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_sbcd<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_scc<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_stop<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_sub<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_suba<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_subi<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_subq<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_subx<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_swap<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_tas<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_trap<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_trapv<M: MemoryAccess + ?Sized>(
        &mut self,
        _: &mut M,
        _: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_tst<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
    ...
    fn instruction_unlk<M: MemoryAccess + ?Sized>(
        &mut self,
        memory: &mut M,
        inst: &Instruction,
    ) -> InterpreterResult {
...
impl<E: CpuDetails, M: MemoryAccess + ?Sized> Execute<E, M> {
    ...
    const EXECUTE: [fn(&mut M68000<E>, &mut M, &Instruction) -> InterpreterResult;
        Isa::_Size as usize] = [
