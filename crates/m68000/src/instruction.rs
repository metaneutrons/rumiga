...
use crate::exception::ACCESS_ERROR;
...
impl Size {
    ...
    pub fn as_word_long(self) -> Self {
        if self == Self::Byte { Self::Word } else { self }
    }
...
impl Operands {
    ...
    /// ILLEGAL, NOP, RESET, RTE, RTR, RTS, TRAPV
    pub fn new_no_operands<M: MemoryIterator + ?Sized>(_: u16, _: &mut M) -> Self {
        Self::NoOperands
    }

    /// ANDI/EORI/ORI CCR/SR, STOP
    pub fn new_immediate<M: MemoryIterator + ?Sized>(_: u16, memory: &mut M) -> Self {
        Self::Immediate(immediate(memory))
    }

    /// ADDI, ANDI, CMPI, EORI, ORI, SUBI
    pub fn new_size_effective_address_immediate<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// BCHG, BCLR, BSET, BTST
    pub fn new_effective_address_count<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// JMP, JSR, MOVE (f) SR CCR, NBCD, PEA, TAS
    pub fn new_effective_address<M: MemoryIterator + ?Sized>(opcode: u16, memory: &mut M) -> Self {
        Self::EffectiveAddress(effective_address(opcode, memory))
    }

    /// CLR, NEG, NEGX, NOT, TST
    pub fn new_size_effective_address<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// CHK, DIVS, DIVU, LEA, MULS, MULU
    pub fn new_register_effective_address<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// MOVEP
    pub fn new_register_direction_size_register_displacement<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// MOVEA
    pub fn new_size_register_effective_address<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// MOVE
    pub fn new_size_effective_address_effective_address<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// EXG
    pub fn new_register_opmode_register<M: MemoryIterator + ?Sized>(
        opcode: u16,
        _: &mut M,
    ) -> Self {
    ...
    /// EXT
    pub fn new_opmode_register<M: MemoryIterator + ?Sized>(opcode: u16, _: &mut M) -> Self {
        let (opmode, reg) = opmode_register(opcode);
        Self::OpmodeRegister(opmode, reg)
    }

    /// TRAP
    pub fn new_vector<M: MemoryIterator + ?Sized>(opcode: u16, _: &mut M) -> Self {
        Self::Vector(vector(opcode))
    }

    /// LINK
    pub fn new_register_displacement<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// SWAP, UNLK
    pub fn new_register<M: MemoryIterator + ?Sized>(opcode: u16, _: &mut M) -> Self {
        Self::Register(register(opcode))
    }

    /// MOVE USP
    pub fn new_direction_register<M: MemoryIterator + ?Sized>(opcode: u16, _: &mut M) -> Self {
        let (dir, reg) = direction_register(opcode);
        Self::DirectionRegister(dir, reg)
    }

    /// MOVEM
    pub fn new_direction_size_effective_address_list<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// ADDQ, SUBQ
    pub fn new_data_size_effective_address<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// Scc
    pub fn new_condition_effective_address<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// DBcc
    pub fn new_condition_register_displacement<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// BRA, BSR
    pub fn new_displacement<M: MemoryIterator + ?Sized>(opcode: u16, memory: &mut M) -> Self {
        Self::Displacement(displacement(opcode, memory))
    }

    /// Bcc
    pub fn new_condition_displacement<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// MOVEQ
    pub fn new_register_data<M: MemoryIterator + ?Sized>(opcode: u16, _: &mut M) -> Self {
        let (reg, data) = register_data(opcode);
        Self::RegisterData(reg, data)
    }

    /// ADD, AND, CMP, EOR, OR, SUB
    pub fn new_register_direction_size_effective_address<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// ADDA, CMPA, SUBA
    pub fn new_register_size_effective_address<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// ABCD, ADDX, SBCD, SUBX
    pub fn new_register_size_mode_register<M: MemoryIterator + ?Sized>(
        opcode: u16,
        _: &mut M,
    ) -> Self {
    ...
    /// CMPM
    pub fn new_register_size_register<M: MemoryIterator + ?Sized>(opcode: u16, _: &mut M) -> Self {
        let (regl, size, regr) = register_size_register(opcode);
        Self::RegisterSizeRegister(regl, size, regr)
    }

    /// ASm, LSm, ROm, ROXm
    pub fn new_direction_effective_address<M: MemoryIterator + ?Sized>(
        opcode: u16,
        memory: &mut M,
    ) -> Self {
    ...
    /// ASr, LSr, ROr, ROXr
    pub fn new_rotation_direction_size_mode_register<M: MemoryIterator + ?Sized>(
        opcode: u16,
        _: &mut M,
    ) -> Self {
...
/// ANDI/EORI/ORI CCR/SR, STOP
pub fn immediate<M: MemoryIterator + ?Sized>(memory: &mut M) -> u16 {
    memory
        .next()
...
/// ADDI, ANDI, CMPI, EORI, ORI, SUBI
pub fn size_effective_address_immediate<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
...
/// BCHG, BCLR, BSET, BTST
pub fn effective_address_count<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> (AddressingMode, u8) {
...
/// JMP, JSR, MOVE (f) SR CCR, NBCD, PEA, TAS
pub fn effective_address<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> AddressingMode {
...
/// CLR, NEG, NEGX, NOT, TST
pub fn size_effective_address<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> (Size, AddressingMode) {
    let eareg = bits(opcode, 0, 2) as u8;
...
/// CHK, DIVS, DIVU, LEA, MULS, MULU
pub fn register_effective_address<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> (u8, AddressingMode) {
    let isa = DECODER[opcode as usize];
...
/// MOVEP
pub fn register_direction_size_register_displacement<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> (u8, Direction, Size, u8, i16) {
...
/// MOVEA
pub fn size_register_effective_address<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
...
/// MOVE
pub fn size_effective_address_effective_address<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> (Size, AddressingMode, AddressingMode) {
    let size = Size::from_move(bits(opcode, 12, 13));
...
/// LINK
pub fn register_displacement<M: MemoryIterator + ?Sized>(opcode: u16, memory: &mut M) -> (u8, i16) {
    let reg = bits(opcode, 0, 2) as u8;
    let disp = memory
        .next()
        .expect("Access error occured when fetching displacement operand.") as i16;
...
/// MOVEM
pub fn direction_size_effective_address_list<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> (Direction, Size, AddressingMode, u16) {
    let list = memory
...
/// ADDQ, SUBQ
pub fn data_size_effective_address<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
...
/// Scc
pub fn condition_effective_address<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> (u8, AddressingMode) {
...
/// DBcc
pub fn condition_register_displacement<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> (u8, u8, i16) {
    let disp = memory
...
/// BRA, BSR
pub fn displacement<M: MemoryIterator + ?Sized>(opcode: u16, memory: &mut M) -> i16 {
...
/// Bcc
pub fn condition_displacement<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> (u8, i16) {
    let mut disp = opcode as i8 as i16;
...
/// ADD, AND, CMP, EOR, OR, SUB
pub fn register_direction_size_effective_address<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> (u8, Direction, Size, AddressingMode) {
    let reg = bits(opcode, 9, 11) as u8;
...
/// ADDA, CMPA, SUBA
pub fn register_size_effective_address<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
) -> (u8, Size, AddressingMode) {
    let reg = bits(opcode, 9, 11) as u8;
...
/// ASm, LSm, ROm, ROXm
pub fn direction_effective_address<M: MemoryIterator + ?Sized>(
    opcode: u16,
    memory: &mut M,
