...
impl AddressingMode {
    ...
    pub fn from_memory<M: MemoryIterator + ?Sized>(
        mode: u16,
        reg: u8,
        size: Option<Size>,
        memory: &mut M,
        ...
        match mode {
            ...
            5 => Self::Ariwd(
                reg,
                memory.next().expect("An Access Error occured in Ariwd.") as i16,
            ),
            6 => Self::Ariwi8(
                reg,
                BriefExtensionWord(memory.next().expect("An Access Error occured in Ariwi8.")),
            ),
            7 => match reg {
                ...
                2 => {
                    ...
                    Self::Pciwd(
                        pc,
                        memory.next().expect("An Access Error occured in Pciwd.") as i16,
                    )
                }
                3 => {
                    let pc = memory.next_address();
                    Self::Pciwi8(
                        pc,
                        BriefExtensionWord(
                            memory.next().expect("An Access Error occured in Pciwi8."),
                        ),
                    )
                }
                4 => {
...
impl core::fmt::LowerHex for AddressingMode {
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            _ => core::fmt::Display::fmt(self, f),
        }
    }
}

impl core::fmt::UpperHex for AddressingMode {
...
impl core::fmt::Display for BriefExtensionWord {
