...
use m68000::instruction::{Direction, Direction::*, Size, Size::*};
use m68000::isa::Isa;
...
fn size_effective_address_immediate(asm: fn(Size, AM, u32) -> Vec<u16>, opcode: u16) {
    for size in [Byte, Word, Long] {
        for imm in [1, 0x106, 0x1_0005, 0x100_0007] {
            ...
            if size.is_long() {
                assert_eq!(
                    &asm(size, AM::Drd(imm as u8), imm),
                    &immediate_long_assemble_am(opcode, imm, AM::Drd(imm as u8))
                );
                assert_eq!(
                    &asm(size, AM::Ari(imm as u8), imm),
                    &immediate_long_assemble_am(opcode, imm, AM::Ari(imm as u8))
                );
                assert_eq!(
                    &asm(size, AM::Ariwpo(imm as u8), imm),
                    &immediate_long_assemble_am(opcode, imm, AM::Ariwpo(imm as u8))
                );
                assert_eq!(
                    &asm(size, AM::Ariwpr(imm as u8), imm),
                    &immediate_long_assemble_am(opcode, imm, AM::Ariwpr(imm as u8))
                );
                assert_eq!(
                    &asm(size, AM::Ariwd(imm as u8, 0), imm),
                    &immediate_long_assemble_am(opcode, imm, AM::Ariwd(imm as u8, 0))
                );
                assert_eq!(
                    &asm(size, AM::Ariwi8(imm as u8, bew), imm),
                    &immediate_long_assemble_am(opcode, imm, AM::Ariwi8(imm as u8, bew))
                );
                assert_eq!(
                    &asm(size, AM::AbsShort(0xFF00), imm),
                    &immediate_long_assemble_am(opcode, imm, AM::AbsShort(0xFF00))
                );
                assert_eq!(
                    &asm(size, AM::AbsLong(0xFF_0000), imm),
                    &immediate_long_assemble_am(opcode, imm, AM::AbsLong(0xFF_0000))
                );
            } else {
                let imm = if size.is_byte() { imm & 0x00FF } else { imm };
                assert_eq!(
                    &asm(size, AM::Drd(imm as u8), imm),
                    &immediate_assemble_am(opcode, imm as u16, AM::Drd(imm as u8))
                );
                assert_eq!(
                    &asm(size, AM::Ari(imm as u8), imm),
                    &immediate_assemble_am(opcode, imm as u16, AM::Ari(imm as u8))
                );
                assert_eq!(
                    &asm(size, AM::Ariwpo(imm as u8), imm),
                    &immediate_assemble_am(opcode, imm as u16, AM::Ariwpo(imm as u8))
                );
                assert_eq!(
                    &asm(size, AM::Ariwpr(imm as u8), imm),
                    &immediate_assemble_am(opcode, imm as u16, AM::Ariwpr(imm as u8))
                );
                assert_eq!(
                    &asm(size, AM::Ariwd(imm as u8, 0), imm),
                    &immediate_assemble_am(opcode, imm as u16, AM::Ariwd(imm as u8, 0))
                );
                assert_eq!(
                    &asm(size, AM::Ariwi8(imm as u8, bew), imm),
                    &immediate_assemble_am(opcode, imm as u16, AM::Ariwi8(imm as u8, bew))
                );
                assert_eq!(
                    &asm(size, AM::AbsShort(0xFF00), imm),
                    &immediate_assemble_am(opcode, imm as u16, AM::AbsShort(0xFF00))
                );
                assert_eq!(
                    &asm(size, AM::AbsLong(0xFF_0000), imm),
                    &immediate_assemble_am(opcode, imm as u16, AM::AbsLong(0xFF_0000))
                );
            }
...
/// BCHG, BCLR, BSET, BTST
fn effective_address_count(
    asm_dynamic: fn(u8, AM) -> Vec<u16>,
    asm_static: fn(AM, u8) -> Vec<u16>,
    opcode: u16,
    is_btst: bool,
) {
    for reg in 0..10 {
        ...
        if reg <= 7 {
            assert_eq!(
                &asm_dynamic(reg, AM::Drd(reg)),
                &assemble_am(opcode_dynamic, AM::Drd(reg))
            );
            assert_eq!(
                &asm_static(AM::Drd(reg), reg),
                &immediate_assemble_am(opcode_static, reg as u16, AM::Drd(reg))
            );
            catch_unwind(|| asm_dynamic(reg, AM::Ard(reg))).unwrap_err();
            catch_unwind(|| asm_static(AM::Ard(reg), reg)).unwrap_err();
            assert_eq!(
                &asm_dynamic(reg, AM::Ari(reg)),
                &assemble_am(opcode_dynamic, AM::Ari(reg))
            );
            assert_eq!(
                &asm_static(AM::Ari(reg), reg),
                &immediate_assemble_am(opcode_static, reg as u16, AM::Ari(reg))
            );
            assert_eq!(
                &asm_dynamic(reg, AM::Ariwpo(reg)),
                &assemble_am(opcode_dynamic, AM::Ariwpo(reg))
            );
            assert_eq!(
                &asm_static(AM::Ariwpo(reg), reg),
                &immediate_assemble_am(opcode_static, reg as u16, AM::Ariwpo(reg))
            );
            assert_eq!(
                &asm_dynamic(reg, AM::Ariwpr(reg)),
                &assemble_am(opcode_dynamic, AM::Ariwpr(reg))
            );
            assert_eq!(
                &asm_static(AM::Ariwpr(reg), reg),
                &immediate_assemble_am(opcode_static, reg as u16, AM::Ariwpr(reg))
            );
            assert_eq!(
                &asm_dynamic(reg, AM::Ariwd(reg, 0)),
                &assemble_am(opcode_dynamic, AM::Ariwd(reg, 0))
            );
            assert_eq!(
                &asm_static(AM::Ariwd(reg, 0), reg),
                &immediate_assemble_am(opcode_static, reg as u16, AM::Ariwd(reg, 0))
            );
            assert_eq!(
                &asm_dynamic(reg, AM::Ariwi8(reg, bew)),
                &assemble_am(opcode_dynamic, AM::Ariwi8(reg, bew))
            );
            assert_eq!(
                &asm_static(AM::Ariwi8(reg, bew), reg),
                &immediate_assemble_am(opcode_static, reg as u16, AM::Ariwi8(reg, bew))
            );
            assert_eq!(
                &asm_dynamic(reg, AM::AbsShort(0xFF00)),
                &assemble_am(opcode_dynamic, AM::AbsShort(0xFF00))
            );
            assert_eq!(
                &asm_static(AM::AbsShort(0xFF00), reg),
                &immediate_assemble_am(opcode_static, reg as u16, AM::AbsShort(0xFF00))
            );
            assert_eq!(
                &asm_dynamic(reg, AM::AbsLong(0xFF_0000)),
                &assemble_am(opcode_dynamic, AM::AbsLong(0xFF_0000))
            );
            assert_eq!(
                &asm_static(AM::AbsLong(0xFF_0000), reg),
                &immediate_assemble_am(opcode_static, reg as u16, AM::AbsLong(0xFF_0000))
            );

            if is_btst {
                assert_eq!(
                    &asm_dynamic(reg, AM::Pciwd(0, 0)),
                    &assemble_am(opcode_dynamic, AM::Pciwd(0, 0))
                );
                assert_eq!(
                    &asm_static(AM::Pciwd(0, 0), reg),
                    &immediate_assemble_am(opcode_static, reg as u16, AM::Pciwd(0, 0))
                );
                assert_eq!(
                    &asm_dynamic(reg, AM::Pciwi8(0, bew)),
                    &assemble_am(opcode_dynamic, AM::Pciwi8(0, bew))
                );
                assert_eq!(
                    &asm_static(AM::Pciwi8(0, bew), reg),
                    &immediate_assemble_am(opcode_static, reg as u16, AM::Pciwi8(0, bew))
                );
                assert_eq!(
                    &asm_dynamic(reg, AM::Immediate(0xFF00)),
                    &assemble_am(opcode_dynamic, AM::Immediate(0xFF00))
                );
                assert_eq!(
                    &asm_static(AM::Immediate(0xFF00), reg),
                    &immediate_assemble_am(opcode_static, reg as u16, AM::Immediate(0xFF00))
                );
            } else {
...
fn effective_address(asm: fn(AM) -> Vec<u16>, opcode: u16, isa: Isa) {
    for reg in 0..10 {
        ...
        if reg <= 7 {
            ...
            assert_eq!(
                &asm(AM::Ariwd(reg, 0)),
                &assemble_am(opcode, AM::Ariwd(reg, 0))
            );
            assert_eq!(
                &asm(AM::Ariwi8(reg, bew)),
                &assemble_am(opcode, AM::Ariwi8(reg, bew))
            );
            assert_eq!(
                &asm(AM::AbsShort(0xFF00)),
                &assemble_am(opcode, AM::AbsShort(0xFF00))
            );
            assert_eq!(
                &asm(AM::AbsLong(0xFF_0000)),
                &assemble_am(opcode, AM::AbsLong(0xFF_0000))
            );
                ...
                || isa == Isa::Pea
            {
                ...
                assert_eq!(
                    &asm(AM::Pciwi8(0, bew)),
                    &assemble_am(opcode, AM::Pciwi8(0, bew))
                );
                if isa != Isa::Jmp && isa != Isa::Jsr && isa != Isa::Pea {
                    assert_eq!(
                        &asm(AM::Immediate(0xFF00)),
                        &assemble_am(opcode, AM::Immediate(0xFF00))
                    );
                }
            } else {
    ...
    for size in [Byte, Word, Long] {
        for reg in 0..10 {
            ...
            if reg <= 7 {
                ...
                assert_eq!(
                    &asm(size, AM::Ariwpo(reg)),
                    &assemble_am(opcode, AM::Ariwpo(reg))
                );
                assert_eq!(
                    &asm(size, AM::Ariwpr(reg)),
                    &assemble_am(opcode, AM::Ariwpr(reg))
                );
                assert_eq!(
                    &asm(size, AM::Ariwd(reg, 0)),
                    &assemble_am(opcode, AM::Ariwd(reg, 0))
                );
                assert_eq!(
                    &asm(size, AM::Ariwi8(reg, bew)),
                    &assemble_am(opcode, AM::Ariwi8(reg, bew))
                );
                assert_eq!(
                    &asm(size, AM::AbsShort(0xFF00)),
                    &assemble_am(opcode, AM::AbsShort(0xFF00))
                );
                assert_eq!(
                    &asm(size, AM::AbsLong(0xFF_0000)),
                    &assemble_am(opcode, AM::AbsLong(0xFF_0000))
                );
    ...
    for reg in 0..10 {
        ...
        if reg <= 7 {
            ...
            if !is_lea {
                ...
                assert_eq!(
                    &asm(reg, AM::Ariwpo(reg)),
                    &assemble_am(opcode, AM::Ariwpo(reg))
                );
                assert_eq!(
                    &asm(reg, AM::Ariwpr(reg)),
                    &assemble_am(opcode, AM::Ariwpr(reg))
                );
            } else {
            ...
            assert_eq!(
                &asm(reg, AM::Ariwd(reg, 0)),
                &assemble_am(opcode, AM::Ariwd(reg, 0))
            );
            assert_eq!(
                &asm(reg, AM::Ariwi8(reg, bew)),
                &assemble_am(opcode, AM::Ariwi8(reg, bew))
            );
            assert_eq!(
                &asm(reg, AM::AbsShort(0xFF00)),
                &assemble_am(opcode, AM::AbsShort(0xFF00))
            );
            assert_eq!(
                &asm(reg, AM::AbsLong(0xFF_0000)),
                &assemble_am(opcode, AM::AbsLong(0xFF_0000))
            );

            assert_eq!(
                &asm(reg, AM::Pciwd(0, 0)),
                &assemble_am(opcode, AM::Pciwd(0, 0))
            );
            assert_eq!(
                &asm(reg, AM::Pciwi8(0, bew)),
                &assemble_am(opcode, AM::Pciwi8(0, bew))
            );
            if !is_lea {
                assert_eq!(
                    &asm(reg, AM::Immediate(0xFF00)),
                    &assemble_am(opcode, AM::Immediate(0xFF00))
                );
            } else {
    ...
    for data in 0..10 {
        for size in [Byte, Word, Long] {
            ...
            if data >= 1 && data <= 8 {
                ...
                let opcode = opcode | (reg as u16) << 9 | Into::<u16>::into(size) << 6;

                assert_eq!(
                    &asm(data, size, AM::Drd(reg)),
                    &assemble_am(opcode, AM::Drd(reg))
                );
                if !size.is_byte() {
                    assert_eq!(
                        &asm(data, size, AM::Ard(reg)),
                        &assemble_am(opcode, AM::Ard(reg))
                    );
                }
                assert_eq!(
                    &asm(data, size, AM::Ari(reg)),
                    &assemble_am(opcode, AM::Ari(reg))
                );
                assert_eq!(
                    &asm(data, size, AM::Ariwpo(reg)),
                    &assemble_am(opcode, AM::Ariwpo(reg))
                );
                assert_eq!(
                    &asm(data, size, AM::Ariwpr(reg)),
                    &assemble_am(opcode, AM::Ariwpr(reg))
                );
                assert_eq!(
                    &asm(data, size, AM::Ariwd(reg, 0)),
                    &assemble_am(opcode, AM::Ariwd(reg, 0))
                );
                assert_eq!(
                    &asm(data, size, AM::Ariwi8(reg, bew)),
                    &assemble_am(opcode, AM::Ariwi8(reg, bew))
                );
                assert_eq!(
                    &asm(data, size, AM::AbsShort(0xFF00)),
                    &assemble_am(opcode, AM::AbsShort(0xFF00))
                );
                assert_eq!(
                    &asm(data, size, AM::AbsLong(0xFF_0000)),
                    &assemble_am(opcode, AM::AbsLong(0xFF_0000))
                );
            } else {
...
fn register_direction_size_effective_address(
    asm: fn(u8, Direction, Size, AM) -> Vec<u16>,
    opcode: u16,
    dirs: &[Direction],
) {
    for reg in 0..10 {
        for dir in ALL_DIRECTIONS {
            for size in [Byte, Word, Long] {
                ...
                if reg <= 7 && dirs.contains(&dir) {
                    ...
                    if dir == DstReg {
                        assert_eq!(
                            asm(reg, dir, size, AM::Drd(reg)).as_slice(),
                            &[opcode | reg as u16]
                        );
                        if size != Byte &&
                        ...
                        {
                            // OR has not this addressing mode.
                            assert_eq!(
                                asm(reg, dir, size, AM::Ard(reg)).as_slice(),
                                &[opcode | 0x8 | reg as u16]
                            );
                        }
                    }
                    assert_eq!(
                        asm(reg, dir, size, AM::Ari(reg)).as_slice(),
                        &assemble_am(opcode, AM::Ari(reg))
                    );
                    assert_eq!(
                        asm(reg, dir, size, AM::Ariwpo(reg)).as_slice(),
                        &assemble_am(opcode, AM::Ariwpo(reg))
                    );
                    assert_eq!(
                        asm(reg, dir, size, AM::Ariwpr(reg)).as_slice(),
                        &assemble_am(opcode, AM::Ariwpr(reg))
                    );
                    assert_eq!(
                        asm(reg, dir, size, AM::Ariwd(reg, 12)).as_slice(),
                        &assemble_am(opcode, AM::Ariwd(reg, 12))
                    );
                    assert_eq!(
                        asm(reg, dir, size, AM::Ariwi8(reg, bew)).as_slice(),
                        &assemble_am(opcode, AM::Ariwi8(reg, bew))
                    );
                    assert_eq!(
                        asm(reg, dir, size, AM::AbsShort(0xFF00)).as_slice(),
                        &assemble_am(opcode, AM::AbsShort(0xFF00))
                    );
                    assert_eq!(
                        asm(reg, dir, size, AM::AbsLong(0xFF_0000)).as_slice(),
                        &assemble_am(opcode, AM::AbsLong(0xFF_0000))
                    );
                    if dir == DstReg {
                        assert_eq!(
                            asm(reg, dir, size, AM::Pciwd(0, 12)).as_slice(),
                            &assemble_am(opcode, AM::Pciwd(0, 12))
                        );
                        assert_eq!(
                            asm(reg, dir, size, AM::Pciwi8(0, bew)).as_slice(),
                            &assemble_am(opcode, AM::Pciwi8(0, bew))
                        );
                        assert_eq!(
                            asm(reg, dir, size, AM::Immediate(16)).as_slice(),
                            &assemble_am_immediate(opcode, 16, size.is_long())
                        );
                    }
                } else {
                    if dirs.len() == 2 {
...
fn register_size_effective_address(asm: fn(u8, Size, AM) -> Vec<u16>, opcode: u16) {
    for reg in 0..10 {
        for size in [Byte, Word, Long] {
            ...
            if reg <= 7 && (size == Word || size == Long) {
                let opcode =
                    opcode | 0x00C0 | if size == Long { 1 << 8 } else { 0 } | (reg as u16) << 9;

                assert_eq!(
                    asm(reg, size, AM::Ari(reg)).as_slice(),
                    &assemble_am(opcode, AM::Ari(reg))
                );
                assert_eq!(
                    asm(reg, size, AM::Ariwpo(reg)).as_slice(),
                    &assemble_am(opcode, AM::Ariwpo(reg))
                );
                assert_eq!(
                    asm(reg, size, AM::Ariwpr(reg)).as_slice(),
                    &assemble_am(opcode, AM::Ariwpr(reg))
                );
                assert_eq!(
                    asm(reg, size, AM::Ariwd(reg, 12)).as_slice(),
                    &assemble_am(opcode, AM::Ariwd(reg, 12))
                );
                assert_eq!(
                    asm(reg, size, AM::Ariwi8(reg, bew)).as_slice(),
                    &assemble_am(opcode, AM::Ariwi8(reg, bew))
                );
                assert_eq!(
                    asm(reg, size, AM::AbsShort(0xFF00)).as_slice(),
                    &assemble_am(opcode, AM::AbsShort(0xFF00))
                );
                assert_eq!(
                    asm(reg, size, AM::AbsLong(0xFF_0000)).as_slice(),
                    &assemble_am(opcode, AM::AbsLong(0xFF_0000))
                );
                assert_eq!(
                    asm(reg, size, AM::Pciwd(0, 12)).as_slice(),
                    &assemble_am(opcode, AM::Pciwd(0, 12))
                );
                assert_eq!(
                    asm(reg, size, AM::Pciwi8(0, bew)).as_slice(),
                    &assemble_am(opcode, AM::Pciwi8(0, bew))
                );
            } else {
...
fn direction_effective_address(asm: fn(Direction, AM) -> Vec<u16>, opcode: u16) {
    for dir in ALL_DIRECTIONS {
        for reg in 0..10 {
            ...
            if reg <= 7 && (dir == Left || dir == Right) {
                let opcode = opcode | if dir == Left { 1 << 8 } else { 0 };

                assert_eq!(
                    asm(dir, AM::Ari(reg)).as_slice(),
                    &assemble_am(opcode, AM::Ari(reg))
                );
                assert_eq!(
                    asm(dir, AM::Ariwpo(reg)).as_slice(),
                    &assemble_am(opcode, AM::Ariwpo(reg))
                );
                assert_eq!(
                    asm(dir, AM::Ariwpr(reg)).as_slice(),
                    &assemble_am(opcode, AM::Ariwpr(reg))
                );
                assert_eq!(
                    asm(dir, AM::Ariwd(reg, -13)).as_slice(),
                    &assemble_am(opcode, AM::Ariwd(reg, -13))
                );
                assert_eq!(
                    asm(dir, AM::Ariwi8(reg, bew)).as_slice(),
                    &assemble_am(opcode, AM::Ariwi8(reg, bew))
                );
                assert_eq!(
                    asm(dir, AM::AbsShort(0x4321)).as_slice(),
                    &assemble_am(opcode, AM::AbsShort(0x4321))
                );
                assert_eq!(
                    asm(dir, AM::AbsLong(0x76543210)).as_slice(),
                    &assemble_am(opcode, AM::AbsLong(0x76543210))
                );
            } else {
...
fn rotation_direction_size_mode_register(
    asm: fn(u16, Direction, Size, bool, u16) -> u16,
    opcode: u16,
) {
        ...
        for dir in ALL_DIRECTIONS {
            for size in [Byte, Word, Long] {
                for ir in [false, true] {
                    for reg in 0..10 {
                        if count_reg <= 7 && reg <= 7 && (dir == Left || dir == Right) {
                            let opcode = opcode
                                | (count_reg as u16) << 9
...
fn assembler_bcc() {
    for cc in 0..16 {
        if cc >= 2 {
            ...
            assert_eq!(
                asm::bcc(get_condition(cc), -10).as_slice(),
                &[bcc | -10i8 as u8 as u16]
            );
            assert_eq!(
                asm::bcc(get_condition(cc), -30_000).as_slice(),
                &[bcc, -30_000i16 as u16]
            );
        } else {
...
fn assembler_dbcc() {
    for cc in 0..16 {
        for reg in 0..10 {
            if reg <= 7 {
                let dbcc = 0x50C8 | (cc as u16) << 8 | reg as u16;
                assert_eq!(
                    asm::dbcc(get_condition(cc), reg, -30_000),
                    [dbcc, -30_000i16 as u16]
                );
            } else {
...
fn assembler_ext() {
    for word_to_long in [false, true] {
        for reg in 0..10 {
            if reg <= 7 {
                let ext = 0x4880 | if word_to_long { 1 << 6 } else { 0 } | reg as u16;
    ...
    for size in [Byte, Word, Long] {
        for reg in 0..10 {
            if reg <= 7 {
                ...
                assert_eq!(
                    asm::r#move(size, AM::Drd(7 - reg), AM::Drd(reg)).as_slice(),
                    &assemble_move_am(r#move, AM::Drd(7 - reg), AM::Drd(reg))
                );
                if !size.is_byte() {
                    assert_eq!(
                        asm::r#move(size, AM::Drd(7 - reg), AM::Ard(reg)).as_slice(),
                        &assemble_move_am(r#move, AM::Drd(7 - reg), AM::Ard(reg))
                    );
                } else {
                    catch_unwind(|| asm::r#move(size, AM::Drd(7 - reg), AM::Ard(reg))).unwrap_err();
                }
                assert_eq!(
                    asm::r#move(size, AM::Ari(7 - reg), AM::Ari(reg)).as_slice(),
                    &assemble_move_am(r#move, AM::Ari(7 - reg), AM::Ari(reg))
                );
                assert_eq!(
                    asm::r#move(size, AM::Ariwd(7 - reg, 30), AM::Ariwd(reg, -30)).as_slice(),
                    &assemble_move_am(r#move, AM::Ariwd(7 - reg, 30), AM::Ariwd(reg, -30))
                );
                assert_eq!(
                    asm::r#move(size, AM::Ariwi8(7 - reg, bew), AM::Ariwi8(reg, bew)).as_slice(),
                    &assemble_move_am(r#move, AM::Ariwi8(7 - reg, bew), AM::Ariwi8(reg, bew))
                );
                assert_eq!(
                    asm::r#move(size, AM::AbsShort(0xFF00), AM::AbsShort(0xFF00)).as_slice(),
                    &assemble_move_am(r#move, AM::AbsShort(0xFF00), AM::AbsShort(0xFF00))
                );
                assert_eq!(
                    asm::r#move(size, AM::AbsLong(0xFF_0000), AM::AbsLong(0xFF_0000)).as_slice(),
                    &assemble_move_am(r#move, AM::AbsLong(0xFF_0000), AM::AbsLong(0xFF_0000))
                );
                assert_eq!(
                    asm::r#move(size, AM::Drd(7 - reg), AM::Pciwd(0, 30)).as_slice(),
                    &assemble_move_am(r#move, AM::Drd(7 - reg), AM::Pciwd(0, 30))
                );
                assert_eq!(
                    asm::r#move(size, AM::Drd(7 - reg), AM::Pciwi8(0, bew)).as_slice(),
                    &assemble_move_am(r#move, AM::Drd(7 - reg), AM::Pciwi8(0, bew))
                );
                assert_eq!(
                    asm::r#move(size, AM::Drd(7 - reg), AM::Immediate(43)).as_slice(),
                    &assemble_move_am_immediate(r#move, AM::Drd(7 - reg), 43, size.is_long())
                );
    ...
    for size in [Byte, Word, Long] {
        for reg in 0..10 {
            if !size.is_byte() && reg <= 7 {
                let bew = BEW::new(false, 7 - reg, false, 15);
                let movea = 0x0040 | if size.is_word() { 3 } else { 2 } << 12 | (reg as u16) << 9;

                assert_eq!(
                    asm::movea(size, reg, AM::Drd(reg)).as_slice(),
                    &assemble_am(movea, AM::Drd(reg))
                );
                assert_eq!(
                    asm::movea(size, reg, AM::Ard(reg)).as_slice(),
                    &assemble_am(movea, AM::Ard(reg))
                );
                assert_eq!(
                    asm::movea(size, reg, AM::Ari(reg)).as_slice(),
                    &assemble_am(movea, AM::Ari(reg))
                );
                assert_eq!(
                    asm::movea(size, reg, AM::Ariwd(reg, -30)).as_slice(),
                    &assemble_am(movea, AM::Ariwd(reg, -30))
                );
                assert_eq!(
                    asm::movea(size, reg, AM::Ariwi8(reg, bew)).as_slice(),
                    &assemble_am(movea, AM::Ariwi8(reg, bew))
                );
                assert_eq!(
                    asm::movea(size, reg, AM::AbsShort(0xFF00)).as_slice(),
                    &assemble_am(movea, AM::AbsShort(0xFF00))
                );
                assert_eq!(
                    asm::movea(size, reg, AM::AbsLong(0xFF_0000)).as_slice(),
                    &assemble_am(movea, AM::AbsLong(0xFF_0000))
                );
                assert_eq!(
                    asm::movea(size, reg, AM::Pciwd(0, 30)).as_slice(),
                    &assemble_am(movea, AM::Pciwd(0, 30))
                );
                assert_eq!(
                    asm::movea(size, reg, AM::Pciwi8(0, bew)).as_slice(),
                    &assemble_am(movea, AM::Pciwi8(0, bew))
                );
                assert_eq!(
                    asm::movea(size, reg, AM::Immediate(43)).as_slice(),
                    &assemble_am_immediate(movea, 43, size.is_long())
                );
            } else {
...
fn assembler_moveusp() {
    for reg in 0..10 {
        for dir in ALL_DIRECTIONS {
            if reg <= 7 && (dir == UspToRegister || dir == RegisterToUsp) {
                let moveusp = 0x4E60 | if dir == UspToRegister { 1 << 3 } else { 0 } | reg as u16;
...
fn assembler_movem() {
    for dir in ALL_DIRECTIONS {
        for size in [Byte, Word, Long] {
            let bew = BEW::new(true, 0, true, 0);
            if !size.is_byte() && (dir == RegisterToMemory || dir == MemoryToRegister) {
                let movem = 0x4880
                    | if dir == MemoryToRegister { 1 << 10 } else { 0 }
                    | if size.is_long() { 1 << 6 } else { 0 };

                assert_eq!(
                    &asm::movem(dir, size, AM::Ari(0), 0xFF),
                    &immediate_assemble_am(movem, 0xFF, AM::Ari(0))
                );
                if dir == MemoryToRegister {
                    assert_eq!(
                        &asm::movem(dir, size, AM::Ariwpo(0), 0xFF),
                        &immediate_assemble_am(movem, 0xFF, AM::Ariwpo(0))
                    );
                    assert_eq!(
                        &asm::movem(dir, size, AM::Pciwd(0, 12), 0xFF),
                        &immediate_assemble_am(movem, 0xFF, AM::Pciwd(0, 12))
                    );
                    assert_eq!(
                        &asm::movem(dir, size, AM::Pciwi8(0, bew), 0xFF),
                        &immediate_assemble_am(movem, 0xFF, AM::Pciwi8(0, bew))
                    );
                } else {
                    assert_eq!(
                        &asm::movem(dir, size, AM::Ariwpr(0), 0xFF),
                        &immediate_assemble_am(movem, 0xFF, AM::Ariwpr(0))
                    );
                }
                assert_eq!(
                    &asm::movem(dir, size, AM::Ariwd(0, 12), 0xFF),
                    &immediate_assemble_am(movem, 0xFF, AM::Ariwd(0, 12))
                );
                assert_eq!(
                    &asm::movem(dir, size, AM::Ariwi8(0, bew), 0xFF),
                    &immediate_assemble_am(movem, 0xFF, AM::Ariwi8(0, bew))
                );
                assert_eq!(
                    &asm::movem(dir, size, AM::AbsShort(0xFF00), 0xFF),
                    &immediate_assemble_am(movem, 0xFF, AM::AbsShort(0xFF00))
                );
                assert_eq!(
                    &asm::movem(dir, size, AM::AbsLong(0xFF_0000), 0xFF),
                    &immediate_assemble_am(movem, 0xFF, AM::AbsLong(0xFF_0000))
                );
            } else {
                    ...
                    for disp in (i16::MIN..=i16::MAX).step_by(8191) {
                            ...
                            && (dir == MemoryToRegister || dir == RegisterToMemory)
                        {
                            let movep = 0x0108
                                ...
                                | addr as u16;
                            assert_eq!(
                                asm::movep(data, dir, size, addr, disp),
                                [movep, disp as u16]
                            );
                        } else {
...
fn assembler_scc() {
    for cond in 0..=0xF {
        ...
        assert_eq!(
            asm::scc(get_condition(cond), AM::Drd(0)).as_slice(),
            &assemble_am(scc, AM::Drd(0))
        );
        assert_eq!(
            asm::scc(get_condition(cond), AM::Ari(0)).as_slice(),
            &assemble_am(scc, AM::Ari(0))
        );
        assert_eq!(
            asm::scc(get_condition(cond), AM::Ariwd(0, -3)).as_slice(),
            &assemble_am(scc, AM::Ariwd(0, -3))
        );
        assert_eq!(
            asm::scc(get_condition(cond), AM::Ariwi8(0, bew)).as_slice(),
            &assemble_am(scc, AM::Ariwi8(0, bew))
        );
        assert_eq!(
            asm::scc(get_condition(cond), AM::AbsShort(0xFF00)).as_slice(),
            &assemble_am(scc, AM::AbsShort(0xFF00))
        );
        assert_eq!(
            asm::scc(get_condition(cond), AM::AbsLong(0xFF_0000)).as_slice(),
            &assemble_am(scc, AM::AbsLong(0xFF_0000))
        );
